...
Application::Application(unsigned int width, unsigned int height, std::wstring title)
	: title_(title)
	, windowWidth_(width)
...
void Application::LoadPipeline(HWND hwnd)
{
...
void Application::LoadAssets()
{
	...
	// fbxモデルのロード
	{
		if (!FbxLoader::Load("Assets/blocks/grass_block.fbx", &fbxVertexInfo_))
		{
	...
	std::vector<D3D12_SUBRESOURCE_DATA> textureSubresources;
	{
		...
		ThrowIfFailed(DxTexFix::PrepareUpload(device_.Get(), scratchImg.GetImages(), scratchImg.GetImageCount(), metadata, textureSubresources));
	}
	...
	// 座標変換マトリクス(CBV)の生成
	{
		...
		DirectX::XMFLOAT3 eye(0, 1.5, -5);
		DirectX::XMFLOAT3 target(0, -0.3, 0);
		DirectX::XMFLOAT3 up(0, 1, -1);
